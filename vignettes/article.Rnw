...
\begin{equation}
q_i = 2\sum_y n_y\sum_A \sum_{x\in X_A}\left[{\hat f}_i^{(y)}(x) \ln{\hat f}_i^{(y)}(x)
-{\hat f}_i(A)\ln{\hat f}_i(A)\right]
\end{equation}
with ${\rm d.f.}=(G-1)({\bar L}_i-1)$, where ${\bar L}_i$ is the number of 
...
&=& \alpha + \sum_i \beta_i(x_i) + \sum_{i<j} \gamma_(x_i,x_j),
\label{eq:F}
\end{eqnarray}
where 
\begin{eqnarray}
...
\begin{equation}
\ln Z_y = \frac{1}{n_y}\sum_{k}\mathbbm{1}(y_k=y)\sum_i 
...
along with the computed significance levels, clearly indicate that the
association of lower \code{Class} status and being \code{Male} with non-survivors.
...
be filtered accordingly. Note the increase in accuracy compared to 
