...
We analyzed 45 data sets from JASPAR database of varying sample
sizes and segment lengths with 
the same protocol. Differences between fully optimized AUC scores and those from non-interacting models (naive Bayes) were most pronounced in 
intermediate ranges of AUC, and were 
independent of segment lengths (Fig.~\ref{fig:tfbs}{\bf a}).
For many data sets, pseudo-likelihood resulted in higher scores than mean field (Fig.~\ref{fig:tfbs}{\bf b}).
...
AUC scores of \pkg{bbl} model trained on 45 transcription factor binding site
