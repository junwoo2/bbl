...
\Abstract{
  ...
  \pkg{bbl}-trained models can be interpreted directly via via their 
...
\citep{ackley_etal}, which for the simple `numeric' predictor model 
($x_i=0,1,\cdots, L_i-1$ with its effects on response ordered accordingly),
\begin{equation}
...
statistical models of the type (\ref{eq:boltzmann}) usually takes the form 
...
is then defined by Eq.~(\ref{eq:boltzmann}). In this case, 
$\Theta_y=\{h_i^{(y)},J_{ij}^{(y)}\}$
...
- \frac{1}{n_y}\sum_{k\in y} P_i(x|y, x_{l\backslash i})
...
In real data where $n \sim {\rm d.f.}$ or less, the matrix inversion is often ill-behaved. It is regularized by interpolation of $\mathsf{C}^{(y)}$ 
...
the logistic regression formula. However, the actual naive Bayes parameter values differ from logistic regression fit.
...
is faster than (\ref{eq:lnz1}) and is preferable for large $n$.
...
(MA0561.1) of binding motif sequences from JASPAR
...
maximum AUC values (Fig.~\ref{fig:jaspar}). The level of performance attainable with non-interacting models, such as position frequency matrix \citep{wasserman_sandelin}, 
...
We analyzed 856 data sets from JASPAR database of varying sample sizes and segment lengths with 
the same protocol. Differences between fully optimized mean field AUC scores and those from non-interacting models (naive Bayes) varied widely, 
largely independent of segment lengths (Fig.~\ref{fig:tfbs}{\bf a}). 
...
\caption{\small AUC scores of \pkg{bbl} model trained on 856 transcription factor binding site sequence data sets from JASPAR \citep{jaspar2018} under the same protocol as in Fig.~\ref{fig:jaspar}.
...
Boltzmann Bayes model-based classifiers applicable to 
heterogeneous, multifactorial predictor data associated with a 
discrete multi-class response variable.
...
numeric predictors. The package \pkg{bbl} extends the basic statistical distribution to accommodate heterogeneous, factor-valued predictors via 
