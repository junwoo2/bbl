...
To preserve the integrity of prediction on validation set, in which the same predictor may 
have factor values not observed in the training set, we do not alter the overall factor levels 
during cross-validation. 
This convention can lead to difficulty in pseudo-likelihood maximization because
optimization based on Eqs.~(\ref{eq:partial}) is ill-defined when any of the predictors are
constant. In such cases, we augment the training data by an extra instance, in which constant predictors 
take other factor levels.
...
Eqs.~(\ref{eq:Cij}), (\ref{eq:mf}) and (\ref{eq:hbari}) to such cases, the single-variable
...
%
<<mnist2>>=
cv <- crossval(mnist, method='mf', eps=0.05)
@
%
...
%
<<mnist3>>=
mnist <- train(mnist, method='mf', eps=0.05)
