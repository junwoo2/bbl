...
configuration) index. We also introduce frequency $w_k$, each of which
is integral number of times each configuration was observed in data, such that
$\sum_k w_k=n_s$ is the total sample size. If data takes the form of 
...
We use symbol $y$ for a particular factor value and 
...
=\sum_{k \in y}w_k\ln P({\bf x}^k|y)+n_y p_y,
\label{eq:Ly}
\end{equation}
where $p_y\equiv P(y)$ is the marginal distribution of $y$ and $n_y=\sum_k w_k$ 
...
\begin{equation}
L_y-n_yp_y=\sum_{k \in y} w_k\ln P({\bf x}^k|y)
...
in each row along with frequency or raw data (one observation per row) we generate
using the utility function \code{freq2raw}:
%
<<raw>>=
library(bbl)
titanic_raw <- freq2raw(titanic, freq=freq)
head(titanic_raw)
summary(titanic_raw)
@
%
...
The logistic regression shown in the previous subsection allowed for
...
The corresponding linear regression coefficients, which have the same functional form
as in Eq.~(\ref{eq:F}), are measures of the {\it difference} in enrichment in the
two response groups, while the two terms whose difference gave the linear regression
...
difference corresponds to the linear regression outcome. For multinomial responses,
therefore, the information content of BB inference outcome grows linearly with the
number of groups, while that of \code{glm}-based regression remains the same.
...
=h_i^{(y)}-h_i$ and
$\Delta J_{ij}^{(y)}=J_{ij}^{(y)}-J_{ij}$; i.e., 
...
@
%
Note that bias parameters printed are differences with respect to the
pooled values, $\Delta h_i^{(y)}(a)=h_i^{(y)}(a)-h_i(a)$, and 
test results are those from likelihood ratio test applied to the
naive Bayes result, Eq.~(\ref{eq:qi}), with the null hypothesis
$\Delta h_i^{(y)}(a)=0$. The tables of bias parameters shown above 
includes those for both of two survival status groups, clearly indicating
enrichment of lower \code{Class} status among non-survivors, 
\code{Female} among survivors, and \code{Adult} among non-survivors.

We now fit an interacting model using \pkg{bbl}:
%
<<survival>>=
...
oldpar <- par(mar=c(6,6,3,3),tck=-0.05)
plot(bfit)
par(oldpar)
@
...
Note that \code{Female} members were strongly depleted (bias parameters; top) and \code{Female} \code{3rd}-class passengers were
over-represented (interactions; bottom left) among non-survivors.

We now retrain using training data and make prediction on test data:
%
<<predict.bbl>>=
...
\code{newdata}. The \code{predict} method returns a data frame 
...
Cross-validation run of \code{Titanic} data in \code{bbl}.}
\end{figure}

It returns an object with a \code{data.frame} of AUCs for multiple \code{lambda} values and optimal values with maximum AUC, which in this case is at 
$\lambda=1\times 10^{-3}$. We use this information
to make prediction as follows:
%
<<pr2>>=
...
Comparison of true parameters and those inferred from pseudo-likelihood BB 
inference. See the text for conditions.}
\end{figure}
...
As shown in Fig.~\ref{fig:mfcv}{\bf a}, prediction AUC is optimized near $\epsilon=0.7$.
...
plus stop signal ('*'), encoded by DNA sequences ($x_i=\sf{a,t,g,c}$).
In nature, three nucleotides combine to encode specific amino acids. 
We will train models attempting to discover this genetic code from data.
%
<<ntaa>>=
set.seed(351)
n <- 2000
...
@
%
We generated random instances of triplet codons above for training.
...
We now cross-validate using \pkg{bbl}:
%
<<aacv>>=
...
Let us compare this model with multinomial regression using \pkg{glmnet}, 
which enables lasso-penalized fitting,
cross-validation, and prediction of generalized linear models
\citep{friedman_etal}.
\setkeys{Gin}{width=0.6\textwidth}
%
<<glmnet>>=
...
x <- data.matrix(dat)
gn <- cv.glmnet(x=x, y=aa, family='multinomial')
ipanel <- as.matrix(expand.grid(b1=1:4, b2=1:4, b3=1:4))
gp <- predict(gn, newx=ipanel)
ghat <- apply(gp[,,1], 1, function(x){names(x)[which.max(x)]})
gscore <- mean(ap==ghat)
gscore
@
%
Note in the above that \pkg{glmnet} requires integer-coded data matrix.
The function \code{cv.glmnet} identifies the optimal lasso penalization
condition and uses it in \code{predict}.
The resulting regression-based score is substantially lower than 
from \pkg{bbl}, suggesting a superior performance of the latter 
in learning tasks similar to the current example.
...
predictors numbering $\sim 100$ or more; specifically 
the MNIST data set (\url{yann.lecun.com/exdb/mnist/})
...
@
%
Note that before calling \code{crossVal}, we removed
predictors without factor variations
(pixels that are always empty) using the utility function 
...
$\epsilon$ values, one can obtain
the profile shown in Fig.~\ref{fig:mnist} (white symbols). 
The substantial jump in 
performance under $\epsilon^*\sim 0.1$ over $\epsilon \rightarrow 0$ (naive Bayes) limit gives a measure of interaction effects.
...
%
<<mnist3, eval=FALSE>>=
mnist <- bbl(y ~ .^2, method='mf', eps=cv$regstar)
...
Set \code{progress.bar = TRUE} to monitor the progress in a slow 
\code{predict} run.

We performed similar cross-validation and test analyses of the full MNIST data 
(training $n=60,\!000$ and test $n=10,\!000$; Fig.~\ref{fig:mnist}, 
red symbols) and obtained the test score of $0.915$ 
(classification error rate $8.4\%$), which compares favorably 
with some of the large-scale neural network algorithms 
...
%
<<jaspar3>>=
ps <- crossVal(y ~ .^2, data=data, method='pseudo', lambda=10^seq(-2,-1,0.2), verbose=0)
ps
mf <- crossVal(y ~ .^2, data=data, method='mf', eps=seq(0.1,0.4,0.1),verbose=0)
mf
@
%
In both cases, there is an optimal, intermediate range of regularization with
maximum AUC (Fig.~\ref{fig:jaspar}). The level of performance attainable with non-interacting models, such as position frequency matrix 
\citep{wasserman_sandelin}, 
...
a trained model for detection of a binding site to within the resolution of
$\sim 3$~base pairs.
...
most pronounced above the intermediate range of AUC, and were 
...
in Fig.~\ref{fig:jaspar}. ({\bf a}) Comparison of naive Bayes (NB; mean field
with $\epsilon=0$) and full mean field (MF) results. 
({\bf b}) Comparison of mean field (MF) and pseudo-likelihood maximization
(pseudo-L) scores. The symbol colors show the segment length of each
