...
\Abstract{
  Non-regression-based inferences, such as discriminant analysis, can account for
  the effect of predictor distributions that may be significant in big data modeling.
  We describe \pkg{bbl}, an \proglang{R} package for Boltzmann Bayes learning,
  which enables a comprehensive supervised learning of the association between
  a large number of discrete factors and multi-level response variables. Its basic 
  underlying statistical model is a (fully visible) Boltzmann machine inferred for each
  distinct response level. The algorithm reduces to the naive Bayes learner when interaction is ignored.
  We illustrate example use cases for various scenarios, ranging from modeling of a relatively small
  set of factors with heterogeneous levels to those with hundreds or more predictors with uniform levels 
  such as image or genomic data sets. We show how \pkg{bbl} explicitly quantifies the
  extra power provided by interactions via higher predictive performance of the model. In comparison 
  to deep learning-based methods such as restricted Boltzmann machines, \pkg{bbl}-trained models offer 
  the advantage of straightforward interpretation via their single-variable and interaction parameters.
}

\Keywords{Supervised learning, Boltzmann machine, naive Bayes, discriminant analysis, \proglang{R}}
\Plainkeywords{Supevised learning, Boltzmann machine, naive Bayes, discriminant analysis, R}
...
Ideally, it would be best to model the joint distribution $P({\bf x},y)$, which 
contains the full information of predictor/response sets, e.g., via maximum likelihood 
\begin{equation}
{\hat \Theta} = \argmax_\Theta \ln P({\bf x},y|\Theta)
\end{equation}
...
is the simplest approach, where the covariance among ${\bf x}$ is ignored via
$P({\bf x}|y)\simeq \prod_i P(x_i|y)$ with ${\bf x}=(x_1,\cdots,x_m)$.

In this paper, we focus on supervised learners taking into account the high-dimensional nature of 
$P({\bf x}|y)$ beyond the naive Bayes-level description. Namely, 
a suitable parametrization is provided by the Boltzmann machine \citep{ackley_etal}, which for the
simple `numeric' predictor model ($x_i=0,1,\cdots, L-1$ with its effect on response ordered accordingly),
\begin{equation}
...
statistical models of the type (\ref{eq:boltzmann}) usually takes the form of {\it restricted} 
Boltzmann machines instead, where (visible) predictors are augmented by 
hidden units and interactions are zero except between visible and hidden units \citep{hinton}. 
...
We refer to such learning/prediction algorithms using (a generalized version of) 
Eq.~(\ref{eq:boltzmann}) for its statistical model as the Boltzmann Bayes (BB) inference.
An implementation specific to genomic single-nucleotide polymorphism (SNP) data 
(two levels--case and control--of $y$ and uniform three-level predictors, i.e., 
allele counts of $x_i= 0,1,2$) have been reported previously 
\citep{woo_etal-2016}. 
However, this software, written in \proglang{C++}, was geared specifically toward 
genome-wide association studies and is not suitable for use in more general settings. 
We introduce an R package \pkg{bbl} (Boltzmann Bayes Learner), which uses \proglang{C++} functions for 
performance, allowing the user to train and test statistical models in a variety of 
...
We consider two cases for parametrization of the first term in Eq.~(\ref{eq:Ly}). First, one may
assume that predictor variables take discrete factor levels, each with distinct effect on responses,
e.g., $x_i=\text{\code{a},\:\code{t},\:\code{g},\:\code{c}}$ for DNA sequence data.
In this case, we parametrize the group-specific predictor distribution by
\begin{equation}
...
\end{equation}
where $L_i$ is the total number of levels in factor $x_i$, which contributes one less parameters to d.f.
because one of the factors can be taken as reference with the rest measured against it. Internally, 
\pkg{bbl} orders factors, assigns codes $a_i=0,\cdots, L_i-1$, and set $h_i^{(y)}(a_i)=
J_{ij}^{(y)}(a_i,a_j)=0$ whenever $a_i=0$ or $a_j=0$.
We refer to $h_i^{(y)}(x)$ and $J_{ij}^{(y)}(x,x^\prime)$ as bias and interaction parameters,
respectively.

A simpler model can be used when factors are ordered and numeric: $x_i=0,1,\cdots, L_i-1$. The `numeric' model
is then defined by Eq.~(\ref{eq:boltzmann}). In this case, $\Theta_y=(h_i^{(y)},J_{ij}^{(y)})$
consists of ${\rm d.f.}= m + m(m-1)/2=m(m+1)/2$ parameters 
[one $h$ and $J$ for each $i$ and $(i,j)$ pairs], where $m$ is the total number of predictors.
A further simplification of numeric model is to limit predictor levels to be binary ($x_i=0,1$).
The package \pkg{BoltzMM} \citep{BoltzMM} adopts this model with the spin variables $s_i=2x_i-1=\pm 1$.
...
Including an $L_2$ penalizer $\lambda$, $L_{iy}$ in Eq.~(\ref{eq:Ly2}) becomes
\begin{equation}
L_{iy}=\sum_{k\in y} \left[{\bar h}_i^{(y)}(x_i^k|x_{j\backslash i})-\ln Z_{iy}(x_{j\backslash i}^k)\right]
-\frac{\lambda}{2}\left[ \sum_x h_i^{(y)}(x)^2+\sum_{j,x<x^\prime} J_{ij}^{(y)}(x,x^\prime)^2\right]
\label{eq:Liy}
\end{equation}
...
- \frac{1}{n_y}\sum_{k\in y} \frac{e^{{\bar h}_i^{(y)}(x|x_{l\backslash i}^k)}}{Z_{iy}(x_{l\backslash i}^k)}
-\lambda h_i^{(y)}(x),
\\
\frac{\partial L_{iy}/n_y}{\partial J_{ij}^{(y)}(x,x^\prime)} &=& {\hat f}_{ij}^{(y)}(x,x^\prime) 
- \frac{1}{n_y}\sum_{k\in y} \mathbbm{1}(x_j^k=x^\prime)
 \frac{e^{{\bar h}_i^{(y)}(x|x_{l\backslash i}^k)}}{Z_{iy}(x_{l\backslash i}^k)}
-\lambda J_{ij}^{(y)}(x,x^\prime),
\end{eqnarray}
\label{eq:partial}
\end{subequations}
where ${\hat f}_i^{(y)}(x)$ and ${\hat h}_{ij}^{(y)}(x,x^\prime)$ are the first and second moments
of predictor values and $\mathbbm{1}(x)$ is the indicator function.
In \pkg{bbl}, Eqs.~(\ref{eq:partial}) are solved in \proglang{C++} functions using a quasi-Newton
method \code{gsl_multimin_fdfminimizer_vector_bfgs2} in GNU Scientific Library 
...
\end{equation}
Equation~(\ref{eq:mf1}) can be interpreted as treating discrete ${\bf x}$ as if it is multivariate normal:
Eq.~(\ref{eq:pxy}) would then be the counterpart of the multivariate normal p.d.f. and 
$-J_{ij}^{(y)}(x,x^\prime)$ would correspond to the precision matrix.
In real data where $n \sim {\rm d.f}$ or less, the matrix inversion is often ill-behaved. 
Regularization is done by interpolation of $\mathsf{C}^{(y)}$ between non-interacting (naive Bayes)
($\epsilon=0$) and fully interacting limits ($\epsilon=1$):
\begin{equation}
\mathsf{C}^{(y)}  \rightarrow \mathsf{\bar C}^{(y)}=(1-\epsilon)\frac{{\rm Tr}\:\mathsf{C}^{(y)}}
...
The field parameters are given in mean field by using an analog of Eq.~(\ref{eq:barh}):
\begin{equation}
{\hat h}_i^{(y)}(x)=
{\bar h}_i^{(y)}(x)
...
($a_i=0$).
Equation~(\ref{eq:mf}) relates the effective field for predictor $x_i$ (the first term on the right)
as the sum of univariate field (left-hand side) and combined mean effects of interactions with other
variables (the second term on the right) \citep{chandler}.
The effective field is related to frequency via Eq.~(\ref{eq:hbari}) because
\begin{equation}
{\hat f}_i^{(y)}(x)
...
For numeric predictors, Eq.~(\ref{eq:hat}) is replaced by
\begin{equation}
{\hat f}_i^{(y)}\equiv \frac{1}{n_y} \sum_{k \in y} x_i^k = 
\frac{\sum_{x=0}^{L_i-1} x\,e^{{\bar h}_i^{(y)} x}}{\sum_{x=0}^{L_i-1} e^{{\bar h}_i^{(y)}x}},
\end{equation}
which needs to be solved numerically first to find ${\bar h}_i^{(y)}$ so that it can be used in
Eq.~(\ref{eq:mf}).

%\subsection{Naive Bayes}
%The naive Bayes model is contained in BB model as a special case where $J_{ij}^{(y)}=0$.
%There are two options for naive Bayes inference in \pkg{bbl}, either without regularization or
%with regularization. With regularization ($\lambda>0$ in Eqs.~(\ref{eq:Liy})-(\ref{eq:partial}), 
%one has the ability to select important predictors and prevent overfitting when the number of
%predictors are large. In \pkg{bbl}, this option is available via the pseudo-likelihood inference
%by setting interaction parameters to zero.
%When $\lambda=0$, which is suitable for models with moderate to small number of predictors, 
%the advantage is that inference possesses analytical solution (\ref{eq:hbari}). 
%This option is available via mean field inference in \pkg{bbl}.
...
=\frac{1}{1+\sum_{y^\prime\ne y}P({\bf x}|y^\prime)p_{y^\prime}/P({\bf x}|y)p_y}.
\label{eq:py}
\end{equation}
For binary response (e.g., coded as $y=0,1$), Eq.~(\ref{eq:py}) reduces to
\begin{equation}
P(y=1|{\bf x})={\rm logistic}[F({\bf x})]=\frac{1}{1+e^{-F({\bf x})}},
\label{eq:py1}
\end{equation}
where 
\begin{eqnarray}
F({\bf x})&=&\ln P({\bf x}|y=1)-\ln P({\bf x}|y=0) + \ln (p_1/p_0)\nonumber\\
&=& \sum_i \left[h_i^{(1)}(x_i)-h_i^{(0)}(x_i)\right]
+\sum_{i<j} \left[ J_{ij}^{(1)}(x_i,x_j) -J_{ij}^{(0)}(x_i,x_j)\right]+
\ln \frac{Z_0 p_1}{Z_1p_0}.
\label{eq:F}
\end{eqnarray}
Therefore, if $J_{ij}^{(y)}(x,x^\prime)=0$ (naive Bayes), Eq.~(\ref{eq:py1}) takes the form of
the logistic regression formula. However, the actual naive Bayes parameter values differ from 
those from logistic regression.
No expression for $P(y|{\bf x})$ simpler than Eq.~(\ref{eq:py}) exists for data with more than two groups.
Two options for computing $Z_y$ are either
\begin{equation}
...
\label{eq:lnzy}
\end{equation}
which arises from the mean field theory. Equation~(\ref{eq:lnzy}) (option \code{mf = TRUE} in \pkg{bbl})
is faster than (\ref{eq:lnz1}) and is preferrable for large $n$.

For a test data set for which the actual group identities $y_k$ of data points are known, 
the prediction score (accuracy) may be defined as
\begin{equation}
...
independent of probability cutoff. In \pkg{bbl}, the predition score
...
We first illustrate BB inference for full multi-response data sets by \pkg{bbl} using the base 
...
@
%
The option \code{y} specifies the column name of response variable.
...
$h_i^{(y)}$ and $J_{ij}^{(y)}$ as lists with argument order $(y,i)$ and $(y,i,j)$, respectively. 
...
@
%
where Eq.~(\ref{eq:py}) is used with ${\bf x}$ from the supplied \code{newdata}.
The \code{newdata} can either contain the response column as in the above, which the program will disregard,
or only the predictor columns. The predictor columns must either have the same order as in the training data 
or be labeled with column names.
...
predictions for validation sets using trained model:
%
<<cv>>=
cv <- crossval(model, method='pseudo', lambda=10^seq(-6,-2,0.5),verbose=0)
cv
@
%
...
Because there is only one response group, we call the main driver \code{mlestimate} of \pkg{bbl} inference 
directly instead of \code{train}:
%
<<mle>>=
...
@
%
Figure~\ref{fig:par} compares the true and inferred parameters. Here, the sample size was large 
enough that no regularization was necessary.

\begin{figure}[t!]
\centering
<<par, echo=FALSE, fig=TRUE, height=4.0, width=4.5>>=
par(mar = c(4,4,1,2),lwd=0.5,cex.axis=0.8,cex.lab=1.0,mgp=c(2.2,0.9,0),tck=-0.03)
...
par(mfrow=c(2,2),mar = c(4,4,2,2),lwd=0.5,cex.axis=0.8,cex.lab=0.9,mgp=c(2.2,0.8,0),tck=-0.03,las=1)
...
\begin{figure}[t!]
\centering
%\includegraphics[scale=0.8]{/mnt/d/bbm/paper/test_script/mnist/mnist_mf}
...
gives a measure of interactin effects.
The relatively small value of $\epsilon^*$ at the optimal condition,
compared to e.g. Fig.~\ref{fig:mfcv}{\bf a}, reflects the sparseness of image data. 
...
Note the slight increase in test score compared to cross-validation score because of the use of full 
...
(training $n=60,\!000$ and test $n=10,\!000$; Fig.~\ref{fig:mnist}; red symbols) and obtained the test 
score of $0.932$ (classification error rate $6.8\%$), which compares favorably with some of the
best-performing large-scale neural network algorithms 
...
Naive Bayes         & Mean field ($\epsilon=0$) & $30.8$ &  \pkg{bbl}                 \\
...
Transcription factors are proteins that bind to specific DNA sequence segment and regulate
gene expression programs.
Public databases, such as JASPAR \citep{jaspar2018}, currently host known transcription factors and their 
binding sequence motifs. Supervised learners allow users to leverage these data sets and search for
bindnig motifs from candidate sequences.

Here, we illustrate such inferences using an example set (MA0001.1) of binding motif sequences from JASPAR
(\url{http://jaspar.genereg.net}):
%
<<jaspar>>=
seq <- read.fasta(system.file('extdata/MA0561.1.fasta',package='bbl'))
head(seq)
dim(seq)
@
%
The data set consists of common nucleotide segments from $n=335$ raw sequences used for motif discovery.
...
@
%
We asess the performance of pseudo-likelihood and mean field inferences below using cross-validation:
%
<<jaspar3>>=
model <- bbl(data=data)
model
ps <- crossval(model, method='pseudo', lambda=10^seq(-8,1,1), verbose=0)
ps
mf <- crossval(model, method='mf', eps=seq(0.1,0.9,0.1),verbose=0)
mf
@
%
In both results, there is an optimal, intermediate range of regularization with similar
maximum AUC values (Fig.~\ref{fig:jaspar}). The level of performance attainable with
non-interaction models, such as position frequency matrix \citep{wasserman_sandelin}, 
corresponds to the $\epsilon=0$ limit in Fig.~\ref{fig:jaspar}{\bf a}.
...
\begin{figure}[t!]
\centering
%\includegraphics[scale=0.8]{/mnt/d/bbm/paper/test_script/tfbs/ex/fig/tfbf}
...
Data set is from \cite{jaspar2018} (sample ID MA0561.1; see text).
({\bf a}) Pseudo-likelihood and ({\bf b}) mean field inferences.}
\label{fig:jaspar}
\end{figure}

We analyzed 857 data sets from JASPAR database of varying sample sizes and segment lengths with 
the same protocol. Differences between fully optimized mean field AUC scores from non-interacting models 
(naive Bayes) varied widely, largely independent of segment lengths (Fig.~\ref{fig:tfbs}{\bf a}). 
For most of data sets, pseudo-likelihood resulted in substantially higher scores than mean field
(Fig.~\ref{fig:tfbs}{\bf b}).

\begin{figure}[t!]
\centering
%\includegraphics[scale=1.0]{/mnt/d/bbm/paper/test_script/tfbs/pipe/tfbs}
\includegraphics[scale=1]{tfbs}
\caption{\small AUC scores of \pkg{bbl} model trained on 857 transcription factor binding site sequence data sets from JASPAR \cite{jaspar2018} under the same
protocol as in Fig.~\ref{fig:jaspar}.
({\bf a}) Comparison of naive Bayes (NB; mean field with $\epsilon=0$) and full mean
field results. ({\bf b}) Comparison of mean field (MF) and pseudo-likelihood 
...
Boltzmann Bayes model-based classifier applicable to heterogeneous, multifactorial predictor 
data associated with a discrete multi-class response variable.
The currently available package \pkg{BoltzMM} available for fitting data into fully visible Boltzmann 
distribution assumes binary numeric predictors for a single, homogeneous group without
response variables. The package \pkg{bbl} extends the basic statistical distribution to accomodate heterogeneous, 
factor-valued predictors via Eq.~(\ref{eq:pxy}), embedding it in a Bayesian classifier for supervised learning.

Compared to more widely applied restricted Boltzmann machine algorithms, BB model
explicitly infers interaction parameters for all pairs of predictors, 
making direct interpretation of trained models possible. Tests on MNIST suggest performances comparable to other deep layer neural network models in 
...
(Fig.~\ref{fig:jaspar}{\bf b}).

%% -- Optional special unnumbered sections -------------------------------------

\section*{Computational details}

Installation of \pkg{bbl} requires GNU Scientific library 
\url{https://www.gnu.org/software/gsl} installed. The results in this paper 
were obtained using \proglang{R}~\Sexpr{paste(R.Version()[6:7], collapse = ".")}. \proglang{R} itself and all packages used are available from the Comprehensive
