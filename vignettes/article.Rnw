...
\label{eq:partial}
\end{subequations}
where ${\hat f}_i^{(y)}(x)$ and ${\hat h}_{ij}^{(y)}(x,x^\prime)$ are the first and 
second moments of predictor values and $\mathbbm{1}(x)$ is the indicator function.
...
predictors. The package \pkg{bbl} extends the basic statistical distribution to accommodate heterogeneous, factor-valued predictors via 
